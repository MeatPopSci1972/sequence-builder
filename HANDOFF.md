...
## VERSION
- Current: 0.0.0
- Bump pattern: html.split('0.0.0').join('0.0.1')
- Release handoff: https://github.com/MeatPopSci1972/sequence-builder/blob/main/releases/v0.0.0/sequence-builder.html
...
- `## VERSION — Current:` — must match `GET /status` → `version` field (0.0.0)
- `## VERSION — Bump pattern:` — must be `html.split('0.0.0').join('0.0.1')`
- `## VERSION — Release handoff URL:` — must point to current version snapshot (0.0.0)
