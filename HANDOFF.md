...
v0.9.102 local, NOT yet on GitHub. The push has not completed.

FIRST ACTION: run in terminal:
  git push && git push --tags

Then confirm releases/v0.9.102/ appears at:
  https://api.github.com/repos/MeatPopSci1972/sequence-builder/contents/releases/v0.9.102

Then wait ~2 min for Pages and verify:
  GET http://localhost:3799/check-pages?v=0.9.102

OPEN BUG — snapshot fires with v0.0.0 during Run All: buildQuery for snapshot step reads state.version before bump has updated it. Fix: same as check-pages — use state.version as authoritative in buildQuery snapshot case. This is why releases/v0.9.100 and v0.9.102 were missing and had to be manually run.

OPEN BUG — README rebase conflict: README.md is generated by POST /generate-readme. Never edit it on GitHub directly — manual edits cause rebase conflicts. HANDOFF.md documents this.
