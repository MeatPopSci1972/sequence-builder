#!/bin/bash
# sb-sync-labels.sh — idempotent label sync for Sequence Builder
#
...
#
# Usage:
#   GITHUB_TOKEN=ghp_yourtoken bash sb-sync-labels.sh
#
...
if [ -z "$GITHUB_TOKEN" ]; then
  ...
  echo "Usage: GITHUB_TOKEN=ghp_yourtoken bash sb-sync-labels.sh"
  exit 1
fi
