...
3. **UI element factories** — deferred. Trigger for promotion: a second consumer of element construction logic appears outside render(). Design decision recorded in CHANGELOG v0.9.68.
...
<!--
  TEMPLATE NOTE (future automation target):
...
- `## FIRST ACTIONS` — gate counts must match `GET /test` ({{store_test_count}}/{{store_test_count}}) and `GET /test-render` ({{render_test_count}}/{{render_test_count}})
- `## VERSION — Current:` — must match `GET /status` → `version` field ({{version}})
- `## VERSION — Bump pattern:` — must be `html.split('{{version}}').join('{{next_version}}')`
- `## VERSION — Release handoff URL:` — must point to current version snapshot ({{version}})
...
- Live demo link — format: `https://MeatPopSci1972.github.io/sequence-builder/releases/{{version}}/sequence-builder.html`
- All-releases link — format: `https://github.com/MeatPopSci1972/sequence-builder/releases`

**Template-tracked fields:**
- Live demo link version segment must match current release ({{version}})
