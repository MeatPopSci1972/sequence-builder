...
const EXPECTED_TBTNS = 6 // Export/Import are now dropdowns (tbtn-demo-trigger); 4 flat buttons replaced by menu items
...
dropdowns.forEach(function (d) {
  ...
  if (wrapIdx === -1) {
    warn('dropdown wrap not found: #' + d.id)
    return
  }
  ...
  if (searchArea.indexOf('class="' + d.menu + '"') === -1) {
    fail(
      'dropdown menu .' + d.menu + ' not found inside #' + d.id + ' -- possible orphaned fragment'
    )
  }
})
...
if (
  ...
  fail(
    '.tbtn-io-menu CSS missing position:absolute -- dropdown items will render in document flow not float below button'
  )
}
